...
function FoodCard({foodItem,showAddPtn=true,handleIncrement,handleDecrement}:FoodCardType) {
  return (
    <div className=" w-48 h-48 rounded-lg  px-2 py-2 col-span-1 border-2 !border-[#2e3131]">
        ...
        <div className="w-full h-3/5 flex flex-col items-center justify-center gap-y-2">
            ...
            <label className=" text-green-500 font-semibold flex gap-x-1 "><img className=" w-3 h-3 !text-green-500 mt-2" src='/cafe-3d/rupee.svg' /> {foodItem.price} <span className="!text-black">/-</span></label>
            {foodItem.selectedQuantity===0 && showAddPtn && <button className=" w-24 !bg-[#5b5a54] rounded-lg py-1 font-semibold text-sm !text-white" onClick={()=>{handleIncrement(foodItem.name)}}>ADD</button>}
            {foodItem.selectedQuantity!==0 && showAddPtn && <div className=" w-full h-9 flex justify-center items-center"><button className=" w-8 h-8 !bg-[#5b5a54] rounded-s-lg !text-white" onClick={()=>{handleDecrement(foodItem.name)}}>-</button><div className=" w-20 h-8 border border-l-0 border-r-0 flex justify-center items-center">{foodItem.selectedQuantity}</div><button className=" w-8 h-8 !bg-[#5b5a54] rounded-e-lg !text-white" onClick={()=>{handleIncrement(foodItem.name)}}>+</button></div>}
            {!showAddPtn && <div className=" w-24 !text-[#5b5a54] rounded-lg py-1 font-semibold text-sm border flex items-center justify-center !border-[#2e3131]">{foodItem.selectedQuantity}</div>}
        </div>
    </div>
  )
}
