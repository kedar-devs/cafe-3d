...
function FoodCard({foodItem,id,handleIncrement,handleDecrement}:FoodCardType) {
  return (
    <div className=" w-48 h-48 rounded-lg bg-white px-2 py-2 col-span-1">
        ...
        <div className="w-full h-3/5 flex flex-col items-center justify-center gap-y-2">
            ...
            {foodItem.selectedQuantity===0 && <button className=" w-24 bg-blue-500 rounded-lg py-1 font-semibold text-sm text-white" onClick={()=>{handleIncrement(foodItem.name)}}>ADD</button>}
            {foodItem.selectedQuantity!==0 && <div className=" w-full h-9 flex justify-center items-center"><button className=" w-8 h-8 bg-green-500 rounded-lg text-white" onClick={()=>{handleIncrement(foodItem.name)}}>+</button><div className=" w-20 h-8 border border-l-0 border-r-0 flex justify-center items-center">{foodItem.selectedQuantity}</div><button className=" w-8 h-8 bg-green-500 rounded-lg text-white" onClick={()=>{handleDecrement(foodItem.name)}}>-</button></div>}
        </div>
    </div>
  )
}
